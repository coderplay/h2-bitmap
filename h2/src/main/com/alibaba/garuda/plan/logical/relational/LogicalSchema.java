...
public class LogicalSchema {
    ...
    public static class LogicalFieldSchema {
        ...
        public LogicalFieldSchema(String alias, LogicalSchema schema,
                byte type, long uid) {
            this.alias = alias;
            this.type = type;
            this.schema = schema;
        ...
        /**
         * Equality is defined as having the same type and either the same
         * schema or both null schema. Alias and uid are not checked.
         */
        ...
        /**
         * Equality is defined as having the same type and either the same
         * schema or both null schema. if compareAlias argument is set to true,
         * alias is also compared.
         * 
         * @param other
         * @param compareAlias
        ...
        public boolean isEqual(Object other, boolean compareAlias) {
            if (other instanceof LogicalFieldSchema) {
                ...
                if (type != ofs.type)
                    return false;
                if (schema == null && ofs.schema == null)
                    return true;
                if (schema == null)
                    return false;
                else
                    return schema.isEqual(ofs.schema, compareAlias);
            } else {
                return false;
            }
        }
        ...
        public String toString(boolean verbose) {
            ...
            if (alias != null)
                aliasToPrint = alias;
            return aliasToPrint + uidString ;
//            return (aliasToPrint + uidString + ":" + DataType
//                    .findTypeName(type));
        }
        ...
        /**
         * Give new value for uid if uid of field schema or those in fields
         * schema of inner schema are -1.
         */
...
//                uid = LogicalExpression.getNextUid();
        ...
        private boolean compatible(LogicalFieldSchema uidOnlyFieldSchema) {
            ...
            if (this.schema != null) {
                ...
                for (int i = 0; i < this.schema.size(); i++) {
                    boolean comp = schema.getField(i).compatible(
                            uidOnlyFieldSchema.schema.getField(i));
                    if (!comp)
                        return false;
                }
            }
        ...
        /**
         * Adds the uid from FieldSchema argument to this FieldSchema If the
         * argument is null, it stamps this FieldSchema with uid
         * 
         * @param uidOnlyFieldSchema
         * @return FieldSchema
         * @throws FrontendException
         */
        public LogicalSchema.LogicalFieldSchema mergeUid(
                LogicalFieldSchema uidOnlyFieldSchema) throws FrontendException {
            if (uidOnlyFieldSchema != null && compatible(uidOnlyFieldSchema)) {
                this.uid = uidOnlyFieldSchema.uid;
                if (this.schema != null) {
                    for (int i = 0; i < this.schema.size(); i++) {
                        schema.getField(i).mergeUid(
                                uidOnlyFieldSchema.schema.getField(i));
                    }
                }
        ...
        public LogicalFieldSchema deepCopy() {
            LogicalFieldSchema newFs = new LogicalFieldSchema(
                    ...
                    schema != null ? schema.deepCopy() : null, type, uid);
            return newFs;
        }
         ...
         * 
         * @param relaxInner
         *            If true, we don't check inner tuple schemas
         * @param relaxAlias
        ...
        public static boolean equals(LogicalFieldSchema fschema,
                LogicalFieldSchema fother, boolean relaxInner,
                boolean relaxAlias) {
...
//                    if (!LogicalSchema.equals(fschema.schema, fother.schema,
//                            false, relaxAlias)) {
//                        return false;
//                    }
//                }
...
//                    // null. That will cause Schema.equals to return false,
//                    // even though we want to view that as true.
//                    if (!(inFs.schema == null || inFs.schema.size() == 0 || (outFs.schema == null && inFs.schema == null))) {
//                        // compare recursively using schema
...
//                    // good
//                } else if ((DataType.isNumberType(inType) || inType == DataType.CHARARRAY)
//                        && ((outType == DataType.CHARARRAY)
//                                || (outType == DataType.BYTEARRAY) || (DataType
//                                    .isNumberType(outType)))) {
//                    // good
//                } else {
//                    return false;
//                }
//            }
         ...
         * If mode==MergeType.LoadForEach or MergeType.LoadForEachInner, take
         * left side if compatible, otherwise, throw exception. If
         * mode==MergeType.UnionInner, if not same type, throw exception, end up
         * with null inner schema. If mode==MergeType.Union, take more specific
         * type.
         * 
         * @param fs1
         *            In Load/Foreach, fs1 is user declared schema; in Union,
         *            fs1 is left side
         * @param fs2
         *            In Load/Foreach, fs1 is inferred schema; in Union, fs1 is
         *            left side
         * @param mode
         *            merge mode
         */
        public static LogicalFieldSchema merge(LogicalFieldSchema fs1, LogicalFieldSchema fs2, MergeMode mode) throws FrontendException {
            ...
            if (mode==MergeMode.LoadForEach) {
                if (fs1==null) throw new FrontendException("We cannot cast into null");
...
//            byte mergedType = DataType.UNKNOWN;
            ...
            if (mode==MergeMode.UnionInner) {
                if (fs1.type!=fs2.type)
                    ...
                    throw new FrontendException("Incompatable field schema: left is \"" + fs1.toString(false) + "\", right is \"" + fs2.toString(false) + "\"");
                else
                    mergedType = fs1.type;
            }
...
//                    throw new FrontendException("Incompatable field schema: declared is \"" + fs1.toString(false) + "\", infered is \"" + fs2.toString(false) + "\"");
...
//                        schema.addField(new LogicalFieldSchema(null,
...
//            }
    ...
    /**
     * Recursively compare two schemas to check if the input schema can be cast
     * to the cast schema
     * 
     * @param inSch
            ...
            if (fs.alias != null && fs.alias.equals(alias)) {
                if (result == null) {
                    ...
                    StringBuilder sb = new StringBuilder(
                            "Found more than one match: " + result.alias + ", "
                                    + fs.alias);
    ...
    /**
     * Given an alias name, find the associated LogicalFieldSchema. If exact
     * name is not found see if any field matches the part of the 'namespaced'
     * alias. eg. if given alias is nm::a , and schema is (a,b). It will return
     * FieldSchema of a. if given alias is nm::a and schema is (nm2::a, b), it
     * will return null
     * 
     * @param alias
     *            Alias to look up.
     ...
     */
    public LogicalFieldSchema getFieldSubNameMatch(String alias)
            throws FrontendException {
        if (alias == null)
            return null;
    ...
    /**
     * Two schemas are equal if they are of equal size and their fields schemas
     * considered in order are equal. This function does not compare the alias
     * of the fields.
     */
    public boolean isEqual(Object other) {
        return isEqual(other, false);
    ...
    /**
     * Two schemas are equal if they are of equal size and their fields schemas
     * considered in order are equal. If compareAlias argument is set to true,
     * the alias of the fields are also compared.
     * 
     * @param other
     * @param compareAlias
     * @return true if equal
    ...
    public boolean isEqual(Object other, boolean compareAlias) {
        if (other != null && other instanceof LogicalSchema) {
            ...
            for (int i = 0; i < size(); i++) {
                if (!getField(i).isEqual(os.getField(i), compareAlias))
                    return false;
            }
            return true;
        } else {
    ...
    public static enum MergeMode {
        LoadForEach, LoadForEachInner, Union, UnionInner
    }
     ...
     * 
     * @param s1
     *            In Load/ForEach, s1 is user declared schema; In Union, s1 is
     *            left side.
     * @param s2
     *            In Load/ForEach, s2 is infered schema; In Union, s2 is right
     *            side.
     * @param mode
     *            We merge schema in Load/Foreach/Union. In Load/Foreach, we
     *            always take s1 if compatible (s1 is set to be user defined
     *            schema), In union, we take more specific type (between numeric
     *            and string, we take string). In the case type mismatch in
     *            s1/s2, we expect TypeCheckingVisitor will fill the gap later.
     * @return a merged schema, or null if the merge fails
     */
    public static LogicalSchema merge(LogicalSchema s1, LogicalSchema s2,
            MergeMode mode) throws FrontendException {
        // If any of the schema is null, take the other party
        if (s1 == null || s2 == null) {
            if (mode == MergeMode.LoadForEach
                    || mode == MergeMode.LoadForEachInner) {
                if (s1 != null)
                    return s1.deepCopy();
                else if (s2 != null)
                    return s2.deepCopy();
        ...
        for (int i = 0; i < s1.size(); i++) {
            ...
            LogicalFieldSchema mergedFS = LogicalFieldSchema.merge(fs1, fs2,
                    mode);
        ...
        if (uidOnlySchema != null) {
            if (size() != uidOnlySchema.size()) {
                throw new FrontendException(
                        "Structure of schema change. Original: "
                                + uidOnlySchema + " Now: " + this);
            }
            for (int i = 0; i < size(); i++) {
    ...
    /**
     * Merges collection of schemas using their column aliases (unlike
     * mergeSchema(..) functions which merge using positions) Schema will not be
     * merged if types are incompatible, as per DataType.mergeType(..) For
     * Tuples and Bags, SubSchemas have to be equal be considered compatible
     * 
     * @param schemas
    ...
    public static LogicalSchema mergeSchemasByAlias(List<LogicalSchema> schemas)
        ...
        // message
        ArrayList<LogicalSchema> mergedSchemas = new ArrayList<LogicalSchema>(
                schemas.size());
    ...
    /**
     * Merges two schemas using their column aliases (unlike mergeSchema(..)
     * functions which merge using positions) Schema will not be merged if types
     * are incompatible, as per DataType.mergeType(..) For Tuples and Bags,
     * SubSchemas have to be equal be considered compatible
     */
    public static LogicalSchema mergeSchemaByAlias(LogicalSchema schema1,
            LogicalSchema schema2) throws FrontendException {
        LogicalSchema mergedSchema = new LogicalSchema();
        ...
        for (LogicalFieldSchema fs1 : schema1.getFields()) {
            ...
            if (fs2 != null) {
                ...
                LogicalFieldSchema mergedFs = LogicalFieldSchema.merge(fs1,
                        fs2, MergeMode.Union);
    ...
    private static void checkNullAlias(LogicalFieldSchema fs,
            LogicalSchema schema) throws FrontendException {
        if (fs.alias == null) {
            throw new FrontendException(
     ...
     * @param other
     * @param relaxInner
     *            if true, inner schemas will not be checked
     * @param relaxAlias
     *            if true, aliases will not be checked
     * @return true if schemas are equal, false otherwise
     */
    public static boolean equals(LogicalSchema schema, LogicalSchema other,
            boolean relaxInner, boolean relaxAlias) {
        ...
        while (i.hasNext()) {
            ...
            if (!relaxInner
                    && !LogicalFieldSchema.equals(myFs, otherFs, false,
                            relaxAlias)) {
                // Compare recursively using field schema
                return false;
            }
    ...
    /***
     * Old Pig schema does not require a tuple schema inside a bag; Now it is
     * required to have that; this method is to fill the gap
     */
    public void normalize() {
